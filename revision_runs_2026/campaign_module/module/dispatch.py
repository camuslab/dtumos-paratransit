...
def ortools_dispatch(active_passenger, empty_vehicle, cost_matrix):
    ...
    # for i in range(A_cnt):
    #     for j in range(B_cnt):
    #         if cost_matrix[i][j] >= 10: # only match within 10km
    #             solver.Add(x[i, j] <= 0)
    ...
    status = solver.Solve()
    if _os.environ.get('TIMING_FILE'):
        with open(_os.environ['TIMING_FILE'], 'a') as _tf:
            _tf.write(f"{len(active_passenger)},{len(empty_vehicle)},{(_time.perf_counter()-_t0)*1000:.2f}\n")
    # Print solution
    A_iloc = []
    B_iloc = []

...
'''Level Of Service'''
def figure_1(base_path, time_range, time_bins, time_single_labels, save_path = None):
    ### Preprocess data
    file_num = len(os.listdir(base_path))
    ...
    for fd_nm in os.listdir(base_path):
        ...
        failure_passengers['time_cat'] = pd.cut(failure_passengers['end_time'], bins=time_bins, labels=time_single_labels, right=False) # failure counts are based on failure time
...
# import pandas as pd
# import plotly.graph_objects as go 
def figure_2(base_path, time_bins, time_single_labels, time_double_labels, save_path = None):
    ### Preprocess data
    file_num = len(os.listdir(base_path))
    ...
    fig_2.add_trace(go.Bar(
        ...
        opacity=0.5,
        showlegend=False     # legend is drawn manually, so keep it off
    ))
    # (b) two line traces
    for col, nm in [
...
# import plotly.graph_objects as go
# from plotly.subplots import make_subplots
def figure_3(base_path, time_range, time_bins, time_single_labels, save_path = None):
    ### Preprocess data
    ...
    # create a 1-row, 2-column subplot with a 4:1 width ratio
    fig_3 = make_subplots(
        rows=1, cols=2,
    ...
    # ── right: distribution Box for the full period ──
    fig_3.add_trace(
        go.Box(
            y=total_waiting_time_inf["waiting_time"],
            showlegend=False,
